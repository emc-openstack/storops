...
class UnityHostTest(TestCase):
    ...
    def test_detach_with_retry_lun_modified_by_another_request(self):
        ...
        # _detach_with_retry retry 6 times with 5s interval, it will take at least
        # (20 - 1) * 5 = 95s to finish the retries.
    ...
    def test_attach_with_retry_lun_modify_by_another_request(self):
        ...
        with mock.patch('storops.unity.resource.host.UnityHost.'
                        '_modify_hlu',
                        new=mock.Mock(side_effect=UnityLunModifyByAnotherRequestException)):
            start = time.time()
            assert_that(calling(host._attach_with_retry).with_args(lun, True),
                        raises(UnityLunModifyByAnotherRequestException))
            end = time.time()
            # _attach_with_retry retry 6 times with 5s interval, it will take at least
            # (6 - 1) * 5 = 25s to finish the retries.
